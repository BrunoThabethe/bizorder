    ...
    <CustomerLayout>
      ...
      <header className="rounded-3xl bg-card p-5 shadow-card">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Order #{order.id.slice(0, 8)}</p>
            <h1 className="mt-1 font-display text-2xl font-bold md:text-3xl">{order.businesses?.name ?? "Provider"}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{order.services?.title ?? "Custom order"}</p>
          </div>
          <div className="flex flex-col items-start gap-2 md:items-end">
            <span className={cn("rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider", STATUS_TONE[status])}>
              {STATUS_LABEL[status]}
            </span>
            <span className="font-display text-2xl font-bold">{formatPrice(Number(order.total), order.currency)}</span>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <section className="rounded-3xl bg-card p-5 shadow-card">
          ...
          {events.length === 0 ? (
            <p className="mt-3 rounded-2xl bg-muted/40 p-4 text-center text-xs text-muted-foreground">
              No updates yet — the provider will post progress here.
            </p>
          ) : (
        ...
        <section className="rounded-3xl bg-card p-5 shadow-card">
          ...
          <div className="mt-3 max-h-80 space-y-2 overflow-y-auto">
            {messages.length === 0 ? (
              <p className="rounded-2xl bg-muted/40 p-4 text-center text-xs text-muted-foreground">No messages yet.</p>
            ) : (
              messages.map((m) => (
                <div
                  key={m.id}
          ...
          {order.addresses ? (
            <div className="mt-5 rounded-2xl bg-muted/40 p-3">
              ...
              <p className="text-xs text-muted-foreground">
                {order.addresses.line1}{order.addresses.line2 ? `, ${order.addresses.line2}` : ""}, {order.addresses.city}
              </p>
            </div>
          ) : null}
        </section>
